...
namespace Mint {
    ...
    public:
        Camera() = default;
        Camera(const glm::mat4& projection, const glm::mat4& unreversed_z)
            : m_projection_matrix(projection), m_unreversed_z_matrix(unreversed_z) {}
        ...
        glm::mat4 GetUnreservedZMatrix() const { return m_unreversed_z_matrix; }

        void SetProjectionMatrix(const glm::mat4& projection, const glm::mat4& unreversed_z) {
            m_projection_matrix = projection; 
            m_unreversed_z_matrix = unreversed_z;
        }
        ...
        void SetPerspectiveProjection(float deg_fov, float aspect_ratio, float zNear, float zFar) {
            ...
            m_unreversed_z_matrix = glm::perspective(glm::radians(deg_fov), aspect_ratio, zNear, zFar);
        }
        ...
        void SetPerspectiveProjectionRad(float rad_fov, float aspect_ratio, float zNear, float zFar) {
            ...
            m_unreversed_z_matrix = glm::perspective(rad_fov, aspect_ratio, zNear, zFar);
        }
        ...
        void SetOrthoProjectionMatrix(const float width, const float height, const float zNear, const float zFar) {
            ...
            m_unreversed_z_matrix = glm::ortho(-width * 0.5f, width * 0.5f, -height * 0.5f, height * 0.5f, zNear, zFar);
        }
        ...
        //     m_unreversed_z_matrix = glm::ortho(left, right, bottom, top, -1.0f, 1.0f);
        // }
        // --------------------------------------------------
    ...
    private:
        ...
        // UnReserved Z
        glm::mat4 m_unreversed_z_matrix = glm::mat4(1.0f); 
    };
}